...
$.ajaxSetup({
    beforeSend: function () {
        //ajax请求之前
        //$(document.body).prepend('<div id="loading" style="position:absolute; background: #f3f3f3 url("Theme/img/loading1.gif") no-repeat 10px 50%;opacity: 0.7;background-position:center center;   width:100%; height:100%; z-index:1"></div>');
    },
    complete: function () {
        //ajax请求完成，不管成功失败
        $("#loading").hide();
...
var erp = {
    ...
    JsonToDateTime: function (jsonDate) {
        ...
        var date = new Date(jsonDate).format("yyyy-MM-dd HH:mm");
        return date;
    },
    newDateFormate: function (jsonDate) {
        if (jsonDate == undefined || jsonDate.length <= 0) return jsonDate;
        var date = new Date(jsonDate).format("yyyy-MM-dd");
        return date;
    },
    newDate: function (jsonDate) {
        if (jsonDate == undefined || jsonDate.length <= 0) return jsonDate;
        var date = new Date(jsonDate).format("yyyy-MM");
        return date;
    },
    //获得访问的路径/Home/Index
    GetPath: function () {
...
; (function ($) {
    ...
    function renderpager(pagenumber, pagecount, pageSize, recordCount, customerText, buttonClickCallback) {
        ...
        $pager += "<div class=\"dataTables_info\"><hd lang_l='lc_pageTotal'>共</hd> " + recordCount + " <hd lang_l='lc_row'>行</hd> ,<hd lang_l='lc_pageSize'>每页</hd>";
        ...
        $pager += "</select><hd lang_l='lc_row'>行</hd></div>";
        $pager += "</div>";
        ...
        if (pagenumber <= 1) {
            $pager += "<li class=\"prev disabled\" data=\"1\"><a href=\"javascript:void(0)\"><hd lang_l='lc_homePage'>首页</hd></a></li>";
        } else {
            $pager += "<li class=\"prev\" data=\"1\"><a href=\"javascript:void(0)\"><hd lang_l='lc_homePage'>首页</hd></a></li>";
        }
        ...
        if (pagenumber >= pagecount) {
            $pager += "<li class=\"next disabled\" data=\"" + pagecount + "\"><a href=\"javascript:void(0)\"><hd lang_l='lc_endPage'>尾页</hd></a></li>";
        } else {
            $pager += "<li class=\"next\" data=\"" + pagecount + "\"><a href=\"javascript:void(0)\"><hd lang_l='lc_endPage'>尾页</hd></a></li>";
        }
        $pager += "</ul>";
        $pager += "</div>";
        $pager += "</div>";
...
; (function ($) {
    ...
    function renderpager(pagenumber, pagecount, pageSize, recordCount, customerText, buttonClickCallback) {
        var $pager = "";
        $pager += "<div style='display: inline-block;'>";
        $pager += "<div class=\"dataTables_paginate paging_bootstrap pagination\" style='margin:0px'>";
        $pager += "<ul class='pager_items'>";
            ...
            success: function (msg) {
                if (msg == "90001") {
                    var submit = function (v, h, f) {
                        if (v == "ok") {
                            window.location.href = "/Home/Index?returnurl=" + erp.GetPath();
                        }
                    }
