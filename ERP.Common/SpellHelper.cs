...
namespace UCMS.Common
{
    public class SpellHelper
    {
        public static string GetSpellCode(string str)
        {
            ...
            {
                var pin = GetTotalPingYin(str.Substring(0,1));
                py = pin.FirstPingYin == null ? "?" : pin.FirstPingYin.FirstOrDefault();
            }
            return py.ToUpper();
        }
        ...
        public static PingYinModel GetTotalPingYin(string str)
        {
            ...
            foreach (var pinyins in totalPingYins)
            {
                ...
                if (result.TotalPingYin == null || result.TotalPingYin.Count <= 0)
                {
