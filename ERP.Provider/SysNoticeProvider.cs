...
namespace UCMS.Provider
{
    public class SysNoticeProvider
    {
        ...
        public int Edit(SysNotice model)
        {
            ...
            {
                return db.SysNotice.Where(c => c.Id==model.Id).Update(r => new SysNotice {
        ...
        {
            return db.SysNotice.Where(c => ids.Contains(c.Id)).Update(r => new SysNotice { IsDelete = (int)Common.EnumModel.EIsDelete.Deleted });
        }
    }
}
